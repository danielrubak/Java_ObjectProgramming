-- 1. Napisz funkcję zysk obliczającą zysk jaki cukiernia uzyskuje ze sprzedaży jednego pudełka czekoladek, zakładając, że zysk ten jest różnicą między ceną pudełka, a kosztem wytworzenia zawartych w nim czekoladek i kosztem opakowania (0,90 zł dla każdego pudełka).
...
CREATE OR REPLACE FUNCTION zysk(idPud char(4))
   RETURNS NUMERIC(7,2) AS
$$

DECLARE cenaPudelka NUMERIC(7, 2);
...
DROP FUNCTION IF EXISTS zyskDzienny(DATE);
CREATE FUNCTION zyskDzienny(dzien DATE)
   RETURNS NUMERIC(7,2) AS
$$
